...
export const TOOL_REFERENCE = {
  ...
  select_page: { group: "Navigation automation", desc: "Selects a page, brings it to the browser foreground, and routes future tool calls there.", args: [
    { name: "page_id", pos: true, req: true, type: "string", desc: "The id of the page to select, from list_pages" },
    { name: "bringToFront", type: "boolean", desc: "Compatibility flag; select_page always brings the chosen page to the browser foreground." },
  ]},
...
const SESSION_COMMANDS = `Session commands:
  browser-use start [--browser-exe <path>] [--extra-flags '<json array>']   # always headed (headless launch is disabled)
  ...
  browser-use status
  browser-use daily-browser [ensure|status] [--no-launch] [--wait-ms=<ms>]   # launch the daily browser if it is closed (the bridge extension lives there)
...
// 未知工具名纠错:前缀命中(navigate → navigate_page)优先,其次是编辑距离相近的拼写错误
export function suggestTools(name, limit = 3) {
  const q = String(name ?? "").toLowerCase();
  if (!q) return [];
  const prefix = [];
  const near = [];
  for (const t of Object.keys(TOOL_REFERENCE)) {
    if (t === q) continue;
    if (t.startsWith(q) || q.startsWith(t)) {
      prefix.push([t, t.length - q.length]);
    } else {
      const d = editDistance(q, t);
      if (d <= Math.max(3, Math.ceil(q.length / 3))) near.push([t, d]);
    }
  }
  const rank = (rows) => rows
    .sort((a, b) => Math.abs(a[1]) - Math.abs(b[1]) || a[0].localeCompare(b[0]))
    .map(([t]) => t);
  return [...rank(prefix), ...rank(near)].slice(0, limit);
}
