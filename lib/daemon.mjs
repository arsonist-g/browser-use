...
import { coreCallTimeoutMs, toolBudgetMs } from "./budget.mjs";
...
import { PipeBrowser, browserWsMethod } from "./pipe-browser.mjs";
import { BridgeServer, toDpCookie } from "./bridge-server.mjs";
import { ensureDailyBrowser } from "./daily-browser.mjs";

// 版本号单一来源 = package.json(status 上报用它判别运行副本,硬编码曾连续两版未同步)
const VERSION = JSON.parse(fs.readFileSync(path.join(path.dirname(url.fileURLToPath(import.meta.url)), "..", "package.json"), "utf8")).version;
const EXT_ALIVE_PROBE_MS = 2000; // Edge 进程探测间隔

const config = loadConfig(true);
ensureHome();

// 监听端口:环境变量优先,与 CLI 的探活/请求端口同源(config.daemon_http_port 仍是默认值)。
// 两侧不一致时,daemon 会绑到 CLI 探不到的端口上,表现为"daemon 启动超时"而日志空白
const daemonPort = Number(process.env.BU_DAEMON_PORT ?? config.daemon_http_port);

/** 会话启动时是否自动拉起日常浏览器:config 可关;测试经 BU_DEV_NO_DAILY_AUTOSTART=1 关闭 */
function dailyAutostartEnabled() {
  return config.daily_browser_autostart !== false && process.env.BU_DEV_NO_DAILY_AUTOSTART !== "1";
}
...
// daemon 曾经"无声消失"过:CLI 用 detached + stdio ignore 拉起它,stdout/stderr 无人接管,
// 于是"崩在 JS 里"与"被人强杀"在日志里长得一模一样(都只剩一段空白),死因无从判断。
// 这三条把死因写进日志文件:崩在 JS → 留下栈;正常退出 → 留下退出码;什么都没有 = 被强杀。
process.on("uncaughtException", (e) => { log("daemon", `uncaughtException: ${e?.stack ?? e}`); process.exit(1); });
process.on("unhandledRejection", (e) => { log("daemon", `unhandledRejection: ${e?.stack ?? e}`); process.exit(1); });
process.on("exit", (code) => log("daemon", `exiting code=${code}`));
...
async function startSession({ headless, browser_exe, extra_flags }) {
  ...
  // 无头启动已禁用(反检测红线):仅测试经 CLI 的 BU_DEV_ALLOW_HEADLESS 放行后才可能为 true
  const headlessFlag = headless === true;
  ...
  // 0. 日常浏览器自启(与下面的启动并行推进):桥扩展活在用户的日常浏览器里,它没开 → 桥离线
  // → 登录态注入为空。拉起的进程归用户所有(detached),等桥的预算见 lib/daily-browser.mjs;
  // 测试经 BU_DEV_NO_DAILY_AUTOSTART=1 关闭(与 BU_DEV_ALLOW_HEADLESS 同一口径,不进 agent 文档)。
  const dailyPromise = (dailyAutostartEnabled()
    ? ensureDailyBrowser({ bridge, exe, log })
    : Promise.resolve({ bridge_connected: false, browser_processes: null, browser_windows: null,
      action: "disabled", launched: false, waited_ms: 0, hint: null })
  ).catch((e) => {
    // 这一步只是"顺手保证登录态来源还在":绝不能把会话启动带崩,更不能变成未处理拒绝
    // (daemon 的 stdout/stderr 被 CLI 丢弃,未处理拒绝会让它无声消失)
    log("daily-browser", `ensure failed: ${e?.message ?? e}`);
    return { bridge_connected: false, browser_processes: null, browser_windows: null,
      action: "error", launched: false, waited_ms: 0, hint: null };
  });
  ...
  updateSession(id, { edge_pid_root: browser.pid, pipe_available: browser.pipeAvailable });
  ...
  return {
    session_id: id, state: "ready", port, login_state: login,
    login_hint: login === "injected" ? null : "aid_ecide",
    pipe_available: browser.pipeAvailable,
    daily_browser: daily,
    // 降级要显式说出来:本会话 PWA 四个工具不可用(Extensions 五个不受影响,走 ws)
    warning: browser.pipeAvailable ? null
      : "the browser did not accept the debugging pipe (fd 3/4) for --remote-debugging-pipe " +
        "(Chromium aborts startup in that case, e.g. when it relaunches itself in a restricted context), " +
        "so this session runs port-only; the 4 PWA tools are unavailable — the Extensions tools still work",
  };
}
...
// ---- 兜底清理 ----
// 删除口径(取代 DEC-010 的"Artifact 不随 profile 删除"):会话结束即完整删除——
// profile、artifacts(截图/trace/快照等占空间大头)、toollog、session.json 一律不留;
// 留着的会话产物没有任何后续用途。删除失败(Windows 文件占用)时目录保留,
// 由下一次 session.clean 按同一口径回收(自愈),不阻塞 stop 响应。
function removeSessionDir(id) {
  try { fs.rmSync(sessionDir(id), { recursive: true, force: true }); } catch { /* 占用:交给 sessions clean */ }
}
...
async function stopSession(id) {
  ...
  try {
    ...
    return { state: "cleaned", summary: null }; // 目录已删(重复 stop):幂等成功
  }
  ...
  // 摘要(删除前统计):toollog 行数 + artifacts 文件数
  let toolCount = 0;
  try {
  ...
  const summary = {
    ...
    duration_ms: Date.now() - Date.parse(doc.created_at),
  };
  // 状态先落盘(目录删除失败时 list/clean 仍见终态,由 clean 回收),随后完整删除
  const done = terminateSession(id, "cleaned", doc.last_error);
  try { fs.rmSync(doc.profile_dir, { recursive: true, force: true }); } catch { /* 占用:交给 sessions clean */ }
  removeSessionDir(id);
...
async function cleanOrphans() {
  ...
  for (const doc of listSessions()) {
    if (liveCores.get(doc.session_id)) continue; // 本 daemon 活跃会话不碰
    if (doc.core_pid) {
    ...
    removeSessionDir(doc.session_id);
...
async function route(op, payload) {
  switch (op) {
    ...
    case "daily.ensure": {
      // 日常浏览器自启(DEC-040):桥扩展活在用户日常浏览器里,它没开 → 桥离线 → 登录态注入为空。
      // launch=false 只读状态:不拉起也不等待(CLI 的 daily-browser status 走这条)。
      const exe = resolveBrowserExe(payload?.browser_exe);
      return await ensureDailyBrowser({
        bridge, exe, log,
        launch: payload?.launch !== false,
        waitMs: payload?.wait_ms,
      });
    }
    case "tool.call": {
      ...
      // 传输预算 = 工具预算 + 余量:两者相等时谁先到点不确定,工具自己的失败原因会被抢走
      const budgetMs = toolBudgetMs(payload.timeout_ms, config.tool_default_timeout_ms);
      return await core.call("tool.call", { tool: payload.tool, args: payload.args ?? {} },
        coreCallTimeoutMs(budgetMs));
    }
    default:
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "POST" && url.pathname === "/pipe/cdp") {
    // 浏览器级 CDP 转发入口(core 的 PWA/Extensions 工具经此调用)。
    // 通道按域名分工(见 pipe-browser.browserWsMethod):Extensions 由 target 层级门控
    // (browser target 可用,page target 报 Method not available)、Target 是 browser 端点
    // 命令,两者都走浏览器级 ws —— port-only 降级态下同样成立(隔离上下文因此在降级态可用);
    // PWA 域需要 AllowUnsafeOperations,只有 pipe 通道给,故仍走 fd3/4,pipe 不可用时如实报
    // PIPE_UNAVAILABLE(core 侧按码透传,不再折叠成 CDP_ERROR)。
    let body = "";
    ...
    req.on("end", async () => {
      ...
      try {
        ...
        const msg = browserWsMethod(method)
          ? await browser.wsCall(method, cdpParams, timeout_ms ?? 30000)
          : await browser.call(method, cdpParams, timeout_ms ?? 30000);
        if (msg.error) {
  ...
  if (req.method === "POST" && url.pathname === "/rpc") {
    ...
    req.on("end", async () => {
      ...
      try {
        ...
        // HTTP 层只做粗分,权威语义在 error.code(lib/error-codes.mjs);码表外的码一律 500
        const httpCode = { INVALID_ARG: 400, NOT_FOUND: 404, UNSUPPORTED: 400, PAGE_BLOCKED: 409, SESSION_NOT_FOUND: 404, BROWSER_NOT_RUNNING: 503, BRIDGE_NOT_CONNECTED: 200, BRIDGE_TIMEOUT: 200, CORE_DEAD: 503, CORE_TIMEOUT: 503, PORT_EXHAUSTED: 500, PIPE_UNAVAILABLE: 503 }[code] ?? 500;
...
// 绑定失败必须留下痕迹:stdio 被 CLI 丢弃(detached + ignore),不写日志就只剩"启动超时"这一句
server.on("error", (e) => {
  log("daemon", `listen failed on 127.0.0.1:${daemonPort}: ${e.message}`);
  process.exit(3);
});

server.listen(daemonPort, "127.0.0.1", () => {
  log("daemon", `listening on 127.0.0.1:${daemonPort}, bridge ws on ${config.bridge_ws_port}`);
