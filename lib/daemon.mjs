...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "POST" && url.pathname === "/pipe/cdp") {
    ...
    // 域按通道分工:Extensions 由 target 层级门控(browser target 可用,page target 报
    // Method not available),走浏览器级 ws —— port-only 降级态下同样成立;PWA 域需要
    // AllowUnsafeOperations,只有 pipe 通道给,故仍走 fd3/4。
    let body = "";
    ...
    req.on("end", async () => {
      ...
      try {
        ...
        const msg = method.startsWith("Extensions.")
  ...
  if (req.method === "POST" && url.pathname === "/rpc") {
    ...
    req.on("end", async () => {
      ...
      try {
        ...
        const httpCode = { INVALID_ARG: 400, SESSION_NOT_FOUND: 404, SESSION_STATE_CONFLICT: 409, BROWSER_NOT_RUNNING: 503, BRIDGE_NOT_CONNECTED: 200, BRIDGE_TIMEOUT: 200, CORE_DEAD: 503, CORE_TIMEOUT: 503, PORT_EXHAUSTED: 500, PIPE_UNAVAILABLE: 503 }[code] ?? 500;
